package com.ngnsoft.ngp.dao.ibatis;
...
import com.ngnsoft.ngp.dao.GenericDao;
import com.ngnsoft.ngp.enums.DataSourceType;
...
public class GenericDaoIbatis implements GenericDao {
    ...
    private void init() {
    	if(sqlMapClientTemplateMap.size() == 0) {
    		for(String dbName : sqlMapClients.keySet()) {
    			...
    			} else if(dbSourceObject instanceof Map) {
					Map<String, SqlMapClient> dbSourceMap = (Map)sqlMapClients.get(dbName);
    				for(String operName : dbSourceMap.keySet()) {
    					if(DataSourceType.UPDATE.value().equalsIgnoreCase(operName)) {
    						SqlMapClientTemplate sqlMapClientTemplate = new SqlMapClientTemplate();
    						sqlMapClientTemplate.setSqlMapClient(dbSourceMap.get(DataSourceType.UPDATE.value()));
    						sqlMapClientTemplateMap.put(dbName + "_" + DataSourceType.UPDATE.value(), sqlMapClientTemplate);
    					} else if (DataSourceType.SELECT.value().equalsIgnoreCase(operName)) {
    						SqlMapClientTemplate sqlMapClientTemplate = new SqlMapClientTemplate();
    						sqlMapClientTemplate.setSqlMapClient(dbSourceMap.get(DataSourceType.SELECT.value()));
    						sqlMapClientTemplateMap.put(dbName + "_" + DataSourceType.SELECT.value(), sqlMapClientTemplate);
						}
    				}
    			}
    ...
    public <T extends BaseModel> List<T> findMulti(T model, Pagination page) {
        return getSqlMapClientTemplate(model.getDbName(), DataSourceType.SELECT).queryForList(
                IbatisDaoUtils.getFindQuery(model.getModelName()), model,
                page.getCurrent_page() * page.getItems_per_page(),
                page.getItems_per_page());
    }
